...
class _ProjectDetailScreenState extends State<ProjectDetailScreen>
  ...
  Future<void> _showAddPhaseSheet(Project project) async {
    final changed = await showModalBottomSheet<bool>(
      ...
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => _ProjectPhaseSheet(
        project: project,
        phases: _standardPhases,
      ),
    );
    if (changed == true) _reload();
  }
  ...
  Future<void> _updatePhaseStatus(
      Project project, ProjectMilestone phase, String status) async {
          ...
          body: NestedScrollView(
                ...
                actions: [
                  PopupMenuButton<String>(
                    ...
                    onSelected: (value) {
                      ...
                      if (value == 'phase') _showAddPhaseSheet(project);
                    ...
                    itemBuilder: (_) => const [
                      ...
                      PopupMenuItem(value: 'phase', child: Text('Add phase')),
                      PopupMenuItem(value: 'team', child: Text('Add team member')),
            ...
            body: TabBarView(
              ...
              children: [
                _ProjectOverview(
                  project: project,
                  onAddPhase: () => _showAddPhaseSheet(project),
...
class _EditProjectSheetState extends State<_EditProjectSheet> {
  ...
  void initState() {
    ...
    _startCtrl = TextEditingController(text: project.startDate == 'Not set' ? '' : project.startDate);
    _dueCtrl = TextEditingController(text: project.dueDate == 'Not set' ? '' : project.dueDate);
      ...
      builder: (_, ctrl) => Container(
        ...
        child: ListView(
          ...
          children: [
            ...
            TextField(controller: _nameCtrl, decoration: const InputDecoration(labelText: 'Project Name')),
            const SizedBox(height: 12),
            TextField(controller: _ownerCtrl, decoration: const InputDecoration(labelText: 'Owner')),
            const SizedBox(height: 12),
            DropdownButtonFormField<String>(
              value: ['Active', 'At Risk', 'On Hold', 'Completed'].contains(_status) ? _status : 'Active',
            ...
            Row(children: [
              Expanded(child: TextField(controller: _startCtrl, decoration: const InputDecoration(labelText: 'Start Date'))),
              const SizedBox(width: 12),
              Expanded(child: TextField(controller: _dueCtrl, decoration: const InputDecoration(labelText: 'Due Date'))),
            ]),
            const SizedBox(height: 12),
            TextField(controller: _descriptionCtrl, maxLines: 4, decoration: const InputDecoration(labelText: 'Description')),
            if (_error != null) ...[
              const SizedBox(height: 12),
      ...
      builder: (_, ctrl) => Container(
        ...
        child: ListView(
          ...
          children: [
            ...
            TextField(controller: _ownerCtrl, decoration: const InputDecoration(labelText: 'Owner')),
            const SizedBox(height: 12),
            TextField(controller: _dueCtrl, decoration: const InputDecoration(labelText: 'Due Date')),
            if (_error != null) ...[
              const SizedBox(height: 12),
              Text(_error!, style: const TextStyle(color: AppColors.error, fontSize: 12)),
            ],
            const SizedBox(height: 20),
            ElevatedButton(onPressed: _saving ? null : _save, child: Text(_saving ? 'Saving...' : 'Add Phase')),
          ],
        ),
      ),
    );
  ...
  Future<void> _save() async {
    ...
    try {
      await CrmApi.instance.addProjectTeamMember(
          widget.project.id, _memberCtrl.text.trim());
      if (mounted) Navigator.pop(context, true);
    } catch (error) {
      if (mounted) setState(() => _error = error.toString());
    } finally {
          ...
          children: [
            const Text('Add Team Member',
                style: TextStyle(
                    ...
                    color: AppColors.slate800)),
            const SizedBox(height: 18),
            TextField(controller: _memberCtrl, decoration: const InputDecoration(labelText: 'Member Name')),
            if (_error != null) ...[
              const SizedBox(height: 12),
              Text(_error!, style: const TextStyle(color: AppColors.error, fontSize: 12)),
            ],
            const SizedBox(height: 20),
            ElevatedButton(onPressed: _saving ? null : _save, child: Text(_saving ? 'Saving...' : 'Add Member')),
          ],
        ),
      ),
    );
